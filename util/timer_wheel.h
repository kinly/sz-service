...
static constexpr time_clock::time_point
__time_point(long long value = 0) noexcept {
...
relative_timestamp(std::chrono::duration<req_tt, period_tt> value) noexcept {
  return std::chrono::duration_cast<duration_tt>(
...
struct clock {
  ...
  clock(time64_t src) : _time64(src) {}
...
struct handle_gen : public singleton<handle_gen> {
private:
  std::mutex _mutex;
  uint16_t _crc = 0;
...
public:
  ...
  timer_handle get() noexcept {
    ...
    std::lock_guard<std::mutex> lock(_mutex); // todo: 待优化
    if (_free_ids.empty()) {
      static uint16_t default_crc = 1;
      return make(next(), default_crc);
    }
    const auto result = _free_ids.front();
    _free_ids.pop();
...
struct event_interface {
  ...
  explicit event_interface(time64_t nxt, time64_t period, uint64_t round,
      ...
      : _next(nxt), _period(period), _round(round),
...
struct event_custom : public event_interface {
  ...
  ~event_custom() override {}

  time64_t next() override {
    event_interface::_next = (tick() + _period) / _precision;
    return _next;
  }

  static std::shared_ptr<event_interface>
  create(time64_t nxt, time64_t period, uint64_t round, timer_callback &&cb,
         timer_stopped_callback &&stopped_cb) {
...
struct event_crontab : public event_interface {
  ...
  util::cron::cronexpr _cronexpr; // cronexpr
  ...
  ~event_crontab() override {}

  time64_t next() override {
    const auto last = event_interface::_next * _precision / 1000;
    event_interface::_next =
        util::cron::cron_next(_cronexpr, last) * 1000 / _precision;
    return event_interface::_next;
  }

  static std::shared_ptr<event_interface>
  create(const std::string &cron_str, timer_callback &&cb,
         timer_stopped_callback &&stopped_cb) {
    try {
      std::shared_ptr<event_crontab> result = std::make_shared<event_crontab>(
          std::forward<timer_callback>(cb),
          std::forward<timer_stopped_callback>(stopped_cb));
      result->_cronexpr = util::cron::make_cron(cron_str);
      result->next();
      return result;
    } catch (util::cron::bad_cronexpr const &ex) {
      // todo: log
      return nullptr;
    }
  }
  ...
  static std::shared_ptr<event_interface>
  create(std::string &&cron_str, timer_callback &&cb,
         timer_stopped_callback &&stopped_cb) {
    try {
      std::shared_ptr<event_crontab> result = std::make_shared<event_crontab>(
          std::forward<timer_callback>(cb),
      ...
      result->next();
      return result;
    } catch (util::bad_cronexpr const &ex) {
      // todo: log
      return nullptr;
    }
  }
...
public:
  timer_wheel() {
    std::lock_guard<mutex_tt> lock(_mutex);
  ...
  inline timer_handle add(const std::string &cron_str,
    ...
    {
      std::lock_guard<mutex_tt> lock(_mutex);
      if (_events.emplace(event_->_handle, event_).second == false) {
        return handle_gen::invalid_handle;
      }
      submit_unsafe(event_);
    }
...
private:
  inline void submit_unsafe(std::shared_ptr<event_interface> evt) {
    ...
    const clock clk1 = {evt->_next};
    const clock clk2 = {_tick};
