...
// https://godbolt.org/z/jvGdjv971
// https://zhuanlan.zhihu.com/p/394454479

inline long
get_month_beginning(uint16_t year,
                    uint16_t month) { // get the first day of a month
  ...
  return ((153 * m + 2) / 5) + 365 * y + (y / 4) - (y / 100) + (y / 400) -
         32044;
}
...
struct MonthLayout {
  MonthLayout(int year, int month) {
    ...
    long this_month_beg{get_month_beginning(year, month)},
        next_month_beg{
            get_month_beginning(year + (month == 12), month % 12 + 1)};
...
static inline void calendar_print() {
  for (int i = 0; i < 4; ++i) {
